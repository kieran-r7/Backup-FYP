...
def graph_stage_5a(x,y,season): 
    ...
    x_axis_int = np.linspace(0, 23, test_no)
    test_int = np.linspace(0, 23, 20)
    ax_new.plot(x_axis_int, interpolated_temp, linewidth=2, markersize=3, label='interp_' +season+'_temp')
...
fig, ax_new = plt.subplots()
interpolated_temp,interpolated_flash_rate = graph_stage_5a(autumn_temp,season_pattern_temp,'AUTUMN')
plt.show()
mi = Correlation_S5(interpolated_temp,interpolated_flash_rate)
plt.show()
